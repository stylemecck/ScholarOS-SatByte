...
const ResumeForm = () => {
  ...
  const generateAIWeightSummary = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/tools/generate-summary', {
  ...
  const enhanceBullet = async (id: string, text: string) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/tools/enhance-bullet', { bulletText: text });
